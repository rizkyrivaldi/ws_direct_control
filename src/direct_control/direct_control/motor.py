...
class OffboardControl(Node):
    def __init__(self):
        # Init node
        super().__init__('motor_output_test')
    ...
    def __init_publisher(self):
        ...
        self.actuator_motor_pub = self.create_publisher(ActuatorMotors, '/fmu/in/actuator_motors', self.qos_profile)
    ...
    """ Send actuator to motor directly """
    def motor_output(self):
        motor = ActuatorMotors()
        motor.control[0] = 2000
        motor.control[1] = 2000
        motor.control[2] = 2000
        motor.control[3] = 2000
        self.actuator_motor_pub.publish(motor)
        self.get_logger().info(f'Motor Control sent: {motor.control[0]} {motor.control[1]} {motor.control[2]} {motor.control[3]}')
